...
def sample_pdf(
    ...
    ----------
    bins : torch.Tensor
        Input bin edges. Shape is (batch_size, num_bins + 1)
    weights : torch.Tensor
        Constant value inside each bin. Shape is (batch_size, num_bins)
    n_samples : int
    ...
    cdf = torch.cat([torch.zeros_like(cdf[..., :1]), cdf], dim=-1) # shape (batch_size, len(bins))

    # Create the positions along the ray to sample from
    if deterministic:
        # Create a uniform set of points from 0 to 1 and expand to shape (batch_size, n_samples)
        u = torch.linspace(0.0, 1.0, steps=n_samples, device=device)
        u = u.expand(*cdf.shape[:-1], n_samples) # shape (batch_size, n_samples)
    else:
        # Create a random set of points between 0 to 1 with shape (batch_size, n_samples)
        u = torch.rand(*cdf.shape[:-1], n_samples, device=device) # shape (batch_size, n_samples)
    ...
    index_bins = torch.stack([below, above], dim=-1)  # shape (batch_size, n_samples, 2)
    ...
    samples = bins_values[..., 0] + t * (bins_values[..., 1] - bins_values[..., 0]) # shape (batch_size, n_samples)
