...
#     render_pose_fn: Callable[[np.ndarray, int, int, np.ndarray], torch.Tensor],
#     val_scene: Scene,
#     out_dir: str | Path,
#     n_frames: int = 120,
#     fps: int = 30,
...
#     elevation_deg: float = 0.0,      # starting elevation (degrees)
...
# ) -> Path | None:
#     """
#     Render a camera path using a provided pose renderer.

#     Parameters
#     ----------
#     render_pose_fn : Callable
#         function(c2w, H, W, K) -> RGB float tensor in [0,1] (linear)
#     val_scene : Scene
#         Scene dataclass with frames (used to fetch base H, W, K and estimate default radius)
#     out_dir : str | Path
#         Output directory for frames (and optional video)
#     n_frames : int,optional
#         Number of frames to render. Defaults to 120
#     fps : int, optional
#         Frames per second for the output video. Defaults to 30
#     world_up : np.ndarray
#         Upwards direction in the world. Autodetect from the transforms if not provided.
#         Defaults to None.
#     path_type : str, optional
#         Type of path to render. Either "circle" or "spiral". Defaults to "circle"
#     radius : float, optional
#         Base path radius; if None, derived from median camera distance in val_scene
#     elevation_deg : float, optional
#         Starting elevation angle in degrees (0 = level with origin). Defaults to 0
#     yaw_range_deg : float, optional
#         Total yaw sweep (e.g., 360 for a full loop). Defaults to 360
#     res_scale : float, optional
#         Multiply base H/W and K by this factor. Defaults to 1.0
#     save_video : bool, optional
#         If True, assemble frames into MP4 at the end. Defaults to True
#     cancel_flag_getter : Callable, optional
#         Optional callback to cancel rendering; checked each frame. Defaults to None.

#     Returns
#     -------
#     Path | None :
#         Path to the MP4 if save_video=True and at least one frame was rendered, else None
#     """
#     out_path_dir = Path(out_dir)
#     out_path_dir.mkdir(parents=True, exist_ok=True)

#     # -------- Base intrinsics from the first validation frame --------
#     base_frame = val_scene.frames[0]
#     base_height, base_width = base_frame.image.shape[:2]
#     base_K = np.array(base_frame.K, dtype=np.float32)

#     # Apply resolution scaling (adjust both resolution and intrinsics)
#     if res_scale != 1.0:
#         render_height = max(1, int(round(base_height * res_scale)))
#         render_width  = max(1, int(round(base_width  * res_scale)))
#         scale_factor = float(res_scale)
#         render_K = base_K.copy()
#         render_K[0, 0] *= scale_factor; render_K[1, 1] *= scale_factor
#         render_K[0, 2] *= scale_factor; render_K[1, 2] *= scale_factor
#     else:
#         render_height, render_width, render_K = base_height, base_width, base_K

#     # --- Auto-detect world 'up' if not provided ---
#     if world_up is None:
#         ups = np.stack([f.c2w[:3, 1] for f in val_scene.frames], axis=0)  # column-1 is 'up' in OpenGL convention
...
#     # Build an orthonormal "horizontal" basis around 'up' for placing the eye
#     # Choose a helper axis not parallel to up
...
#     forward_plane = np.cross(right, up_vec); forward_plane /= (np.linalg.norm(forward_plane) + 1e-8)
#     # Now {right, up_vec, forward_plane} is an ONB; the "horizontal" plane is spanned by {right, forward_plane}.


#     # -------- Estimate a reasonable base radius if not provided --------
#     if radius is None:
#         cam_positions = np.stack([f.c2w[:3, 3] for f in val_scene.frames], axis=0)  # (N,3)
#         median_radius = np.median(np.linalg.norm(cam_positions, axis=1))
#         if not np.isfinite(median_radius) or median_radius < 1e-3:
#             median_radius = 4.0  # fallback sane default for Blender synthetic data
#         radius = float(median_radius)

#     # Convert angles to radians for computation
#     start_elevation_rad = np.deg2rad(float(elevation_deg))
#     total_yaw_rad = np.deg2rad(float(yaw_range_deg))

#     # -------- Spiral path parameters (gentle ramps) --------
#     # These constants keep the "spiral" path subtle by default.
...
#     end_elevation_rad = np.deg2rad(elevation_deg + spiral_elevation_delta_deg)
...
#     # -------- Render frames along the chosen path --------
#     frame_filepaths: list[Path] = []
#     for frame_index in tqdm(range(int(n_frames))):
#         if cancel_flag_getter and cancel_flag_getter():
#             break

#         # Normalized progress in [0,1]
#         t01 = frame_index / max(1, (n_frames - 1))

#         # Yaw angle (negative for clockwise to match previous behavior)
#         yaw_rad = -total_yaw_rad * t01

#         # Choose eye position by path type
#         if path_type_lc == "circle":
#             # Fixed radius and elevation; eye moves around a tilted circle
#             current_radius = radius
#             current_elevation = start_elevation_rad

#         else:  # "spiral"
#             # Radius gently increases; elevation gently rises
#             current_radius = radius * (1.0 + spiral_radius_growth_fraction * t01)
#             current_elevation = (1.0 - t01) * start_elevation_rad + t01 * end_elevation_rad

#         # Place camera on a "tilted" circle: split into in-plane radius + vertical height along up
#         r_in_plane = current_radius * np.cos(current_elevation)
#         h_along_up = current_radius * np.sin(current_elevation)
#         eye = (r_in_plane * (np.cos(yaw_rad) * right + np.sin(yaw_rad) * forward_plane)
#                + h_along_up * up_vec)

#         # Look-at pose using the SAME convention as get_camera_rays (OpenGL: columns = [right, up, -forward])
#         c2w = _look_at(eye=eye, target=np.zeros(3, dtype=np.float32), up=up_vec)

#         # Render this pose: expects linear RGB in [0,1]
#         rgb_linear = render_pose_fn(c2w, render_height, render_width, render_K)
#         rgb_srgb   = linear_to_srgb(rgb_linear)

#         # Write PNG to disk
#         frame_path = out_path_dir / f"frame_{frame_index:05d}.png"
#         save_rgb_png(rgb_srgb, frame_path)
#         frame_filepaths.append(frame_path)

#     # -------- Optionally assemble MP4 from frames --------
#     if save_video and frame_filepaths:
#         video_path = out_path_dir / "camera_path.mp4"
#         frames_uint8 = [imageio.imread(p) for p in frame_filepaths]
#         imageio.mimwrite(video_path, frames_uint8, fps=int(fps), codec="libx264", quality=8)
#         return video_path
...
#     n_frames: int,
#     path_type: str = "circle",          # "circle" | "spiral"
#     radius: float | None = None,        # if None, use median cam distance
#     elevation_deg: float = 0.0,
#     yaw_range_deg: float = 360.0,
#     res_scale: float = 1.0,
#     world_up: np.ndarray | None = None, # if None, average +Y from dataset
# ) -> Tuple[list[np.ndarray], int, int, np.ndarray]:
#     """
#     Returns:
#       poses: list of (4,4) c2w matrices (OpenGL-style: camera looks down -Z, Y-up)
#       H, W : output resolution
#       K    : intrinsics scaled by res_scale
#     """
#     # Base intrinsics/resolution from the first frame
...
#     # Determine radius (median distance of validation cameras)
#     if radius is None:
...
#     # Auto-detect world up: average +Y of c2w matrices
#     if world_up is None:
#         ys = []
...
#             ys.append(y)
#         up = np.mean(np.stack(ys, axis=0), axis=0)
#         if np.linalg.norm(up) < 1e-6:
#             up = np.array([0.0, 1.0, 0.0], dtype=np.float32)
#         world_up = (up / (np.linalg.norm(up) + 1e-8)).astype(np.float32)
#     else:
#         world_up = np.asarray(world_up, dtype=np.float32)
#         world_up /= (np.linalg.norm(world_up) + 1e-8)

#     elev = np.deg2rad(float(elevation_deg))
#     yaw_total = np.deg2rad(float(yaw_range_deg))
#     thetas = np.linspace(0.0, yaw_total, num=int(n_frames), endpoint=False, dtype=np.float32)

#     # Gentle spiral parameters (same as before)
#     spiral_radius_gain = 1.25
#     spiral_elev_gain_deg = 10.0
#     target = np.zeros(3, dtype=np.float32)

#     def _look_at_np(eye: np.ndarray, target: np.ndarray, up: np.ndarray) -> np.ndarray:
#         f = target - eye; f = f / (np.linalg.norm(f) + 1e-8)
#         s = np.cross(f, up); s = s / (np.linalg.norm(s) + 1e-8)
#         u = np.cross(s, f)
#         R = np.stack([s, u, -f], axis=1).astype(np.float32)  # columns are basis
#         M = np.eye(4, dtype=np.float32)
#         M[:3, :3] = R
#         M[:3, 3] = eye.astype(np.float32)
#         return M
...
#             r = float(radius) * (1.0 + (spiral_radius_gain - 1.0) * t)
#             elev_i = elev + np.deg2rad(spiral_elev_gain_deg) * t
#         else:
#             r = float(radius)
#             elev_i = elev
#         eye = np.array([
#             r * np.cos(th) * np.cos(elev_i),
#             r * np.sin(elev_i),
#             r * np.sin(th) * np.cos(elev_i),
#         ], dtype=np.float32)
#         poses.append(_look_at_np(eye, target, world_up))
